...
Lua Script                Go Runtime              Network
   ↓                          ↓                       ↓
response.body = "text" → writer.Write(...) → HTTP Connection
response:Write(...)    → writer.Write(...) → HTTP Connection
response:WriteHeader() → writer.WriteHeader() → HTTP Headers Sent
```

### Important Notes

⚠️ **Write-only properties**: `response.body` and `response.status` cannot be read (return `nil`)
...
10. **response.body is write-only**: Cannot read `response.body` or `response.status` - they return `nil`
11. **Streaming-ready**: Every write goes directly to network - perfect for streaming responses
12. **Performance**: Zero buffering means minimal memory usage and immediate data transmission
