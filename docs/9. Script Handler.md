...
The script handler supports two API styles for building responses:

### 1. Table-based API (Simple)

Direct property assignment, similar to Lua table manipulation:
...
### 2. Go-style API (Method-based)

Methods that mirror Go's `http.ResponseWriter` interface:
...
**Both styles can be mixed in the same script** and provide the same functionality. Choose the style that fits your preference or use case.

### Internal Architecture

Both API styles write directly to the same underlying Go data structures:

- **Efficient**: All operations (table assignments and method calls) write directly to Go `http.Header` and byte slices, not to intermediate Lua tables
- **Single source of truth**: Response data is stored in Go structures (`statusCode`, `headers`, `body`) throughout script execution
- **Transparent**: Lua metatables provide seamless property access while maintaining Go storage
- **Memory efficient**: Data lives in Go's memory space, not the Lua VM

This means both APIs are equally performant and you can freely mix them without any overhead.
...
**Mixed API styles:**
```lua
-- Use both styles in the same script - they write to the same Go structures
...
-- All operations above directly modified Go's http.Header and body slice
```
...
9. **Mix API styles freely**: Both table-based and method-based APIs write to the same Go structures, so use whichever is more convenient for your task
10. **Performance**: Both API styles are equally efficient - all operations write directly to Go memory, not Lua tables
