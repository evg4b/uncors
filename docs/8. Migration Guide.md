...
**New Configuration (v0.8.x) - Using Lua Script:**
...
1. **Remove** the `fake` and `seed` properties from mock response configuration
2. **Add** a `script` field with inline Lua code
3. **Use** the [fakedata](https://github.com/lucapette/fakedata) command-line tool to generate fake data

#### Using fakedata CLI Tool

Install the `fakedata` tool:
...
Generate fake data with the desired fields:
...
For more information about the Script Handler feature, see the [Script Handler documentation](./9.%20Script%20Handler.md).
