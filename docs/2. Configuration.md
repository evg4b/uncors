...
| --------- | ----- | -------------------------------------------------------------------------------- |
| `--from`  | `-f`  | Source host with protocol (e.g., `http://localhost`)                             |
...
| Parameter      | Short | Description                                |
| -------------- | ----- | ------------------------------------------ |
| `--http-port`  | `-p`  | HTTP server listening port (default: 80)   |
| `--https-port` | `-s`  | HTTPS server listening port (default: 443) |
...
**Note:** CLI parameters override configuration file settings.
...
| ------------ | ------- | ------- | ----------------------------------------------- |
| `http-port`  | integer | 80      | HTTP server listening port                      |
| `https-port` | integer | 443     | HTTPS server listening port                     |
...
This configuration forwards all requests from `http://localhost` to `https://github.com`. Additional features like mocking and static file serving can be configured per mapping. See [Response Mocking](./3.-Response-mocking) and [Static File Serving](./4.-Static-file-serving) for details.
...
**Note:** UNCORS adds standard CORS headers to all responses. Custom headers specified here will override the defaults.
...
All requests matching the `*.local.com` pattern are forwarded to the same target:

| Local request                      | Target request                  |
| ---------------------------------- | ------------------------------- |
| `http://raw.local.com`             | `https://github.com`            |
| `http://raw.local.com/api/info`    | `https://github.com/api/info`   |
| `http://docs.local.com`            | `https://github.com`            |
| `http://docs.local.com/index.html` | `https://github.com/index.html` |
...
| Local request                      | Target request                       |
| ---------------------------------- | ------------------------------------ |
| `http://raw.local.com`             | `https://raw.github.com`             |
| `http://raw.local.com/api/info`    | `https://raw.github.com/api/info`    |
| `http://docs.local.com`            | `https://docs.github.com`            |
| `http://docs.local.com/index.html` | `https://docs.github.com/index.html` |
...
> [!NOTE]
> HTTPS server functionality is only activated when at least one mapping uses the `https://` scheme.
