...
This configuration forwards all requests from `http://localhost:8080` to `https://github.com`. The port is specified in the `from` URL and defaults to 80 for HTTP and 443 for HTTPS if omitted. Additional features like mocking and static file serving can be configured per mapping. See [Response Mocking](./3.-Response-mocking) and [Static File Serving](./4.-Static-file-serving) for details.
