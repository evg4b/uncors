...
func TestPrintresponse(t *testing.T) {
	t.Run("should correctly format", func(t *testing.T) {
		tests := []struct {
		...
		}{
			{
				...
				response: makeRospoceURL(100, "POST", "/api/info"),
			...
			},
			{
				...
				response: makeRospoceURL(200, "GET", "/help"),
			...
			},
			{
				...
				response: makeRospoceURL(300, "PATCH", "/api/user"),
			...
			},
			{
				...
				response: makeRospoceURL(400, "DELETE", "/api/user/repmission"),
			...
			},
			{
				...
				response: makeRospoceURL(500, "POST", "/"),
	...
	t.Run("should panic for status code less then 100", func(t *testing.T) {
		assert.Panics(t, func() {
			responseprinter.Printresponse(makeRospoceURL(50, "GET", "/"))
		})
	})

	t.Run("should panic for status codegreat then 599", func(t *testing.T) {
		assert.Panics(t, func() {
			responseprinter.Printresponse(makeRospoceURL(600, "GET", "/"))
		})
	})
}
