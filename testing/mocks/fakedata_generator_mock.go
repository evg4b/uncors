...
// GeneratorMock implements mm_fakedata.Generator
...
// NewGeneratorMock returns a mock for mm_fakedata.Generator
...
type GeneratorMockGenerateExpectation struct {
	...
	expectationOrigins GeneratorMockGenerateExpectationOrigins
...
// GeneratorMockGenerateOrigins contains origins of expectations of the Generator.Generate
type GeneratorMockGenerateExpectationOrigins struct {
	origin     string
	originNode string
	originSeed string
}
...
func (mmGenerate *mGeneratorMockGenerate) ExpectNodeParam1(node *mm_fakedata.Node) *mGeneratorMockGenerate {
	...
	mmGenerate.defaultExpectation.expectationOrigins.originNode = minimock.CallerInfo(1)
...
func (mmGenerate *mGeneratorMockGenerate) ExpectSeedParam2(seed uint64) *mGeneratorMockGenerate {
	...
	mmGenerate.defaultExpectation.expectationOrigins.originSeed = minimock.CallerInfo(1)
...
func (mmGenerate *mGeneratorMockGenerate) When(node *mm_fakedata.Node, seed uint64) *GeneratorMockGenerateExpectation {
	...
	expectation := &GeneratorMockGenerateExpectation{
		...
		expectationOrigins: GeneratorMockGenerateExpectationOrigins{origin: minimock.CallerInfo(1)},
	}
...
// Generate implements mm_fakedata.Generator
	...
	if mmGenerate.GenerateMock.defaultExpectation != nil {
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.node != nil && !minimock.Equal(*mm_want_ptrs.node, mm_got.node) {
				mmGenerate.t.Errorf("GeneratorMock.Generate got unexpected parameter node, expected at\n%s:\nwant: %#v\n got: %#v%s\n",
					mmGenerate.GenerateMock.defaultExpectation.expectationOrigins.originNode, *mm_want_ptrs.node, mm_got.node, minimock.Diff(*mm_want_ptrs.node, mm_got.node))
			}

			if mm_want_ptrs.seed != nil && !minimock.Equal(*mm_want_ptrs.seed, mm_got.seed) {
				mmGenerate.t.Errorf("GeneratorMock.Generate got unexpected parameter seed, expected at\n%s:\nwant: %#v\n got: %#v%s\n",
					mmGenerate.GenerateMock.defaultExpectation.expectationOrigins.originSeed, *mm_want_ptrs.seed, mm_got.seed, minimock.Diff(*mm_want_ptrs.seed, mm_got.seed))
			}

		} else if mm_want != nil && !minimock.Equal(*mm_want, mm_got) {
			mmGenerate.t.Errorf("GeneratorMock.Generate got unexpected parameters, expected at\n%s:\nwant: %#v\n got: %#v%s\n",
...
func (m *GeneratorMock) MinimockGenerateInspect() {
	for _, e := range m.GenerateMock.expectations {
		if mm_atomic.LoadUint64(&e.Counter) < 1 {
			m.t.Errorf("Expected call to GeneratorMock.Generate at\n%s with params: %#v", e.expectationOrigins.origin, *e.params)
		}
	}
	...
	if !m.GenerateMock.invocationsDone() && afterGenerateCounter > 0 {
		m.t.Errorf("Expected %d calls to GeneratorMock.Generate at\n%s but found %d calls",
