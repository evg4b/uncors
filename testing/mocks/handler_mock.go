...
// HandlerMock implements contracts.Handler
...
// NewHandlerMock returns a mock for contracts.Handler
...
// ServeHTTP implements contracts.Handler
	...
	if mmServeHTTP.ServeHTTPMock.defaultExpectation != nil {
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.writer != nil && !minimock.Equal(*mm_want_ptrs.writer, mm_got.writer) {
				mmServeHTTP.t.Errorf("HandlerMock.ServeHTTP got unexpected parameter writer, want: %#v, got: %#v%s\n", *mm_want_ptrs.writer, mm_got.writer, minimock.Diff(*mm_want_ptrs.writer, mm_got.writer))
			}

			if mm_want_ptrs.request != nil && !minimock.Equal(*mm_want_ptrs.request, mm_got.request) {
				mmServeHTTP.t.Errorf("HandlerMock.ServeHTTP got unexpected parameter request, want: %#v, got: %#v%s\n", *mm_want_ptrs.request, mm_got.request, minimock.Diff(*mm_want_ptrs.request, mm_got.request))
			}
...
func (m *HandlerMock) MinimockServeHTTPInspect() {
	...
	if !m.ServeHTTPMock.invocationsDone() && afterServeHTTPCounter > 0 {
		m.t.Errorf("Expected %d calls to HandlerMock.ServeHTTP but found %d calls",
			mm_atomic.LoadUint64(&m.ServeHTTPMock.expectedInvocations), afterServeHTTPCounter)
	}
}
