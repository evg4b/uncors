...
type WriterMockWriteExpectation struct {
	...
	expectationOrigins WriterMockWriteExpectationOrigins
...
func (mmWrite *mWriterMockWrite) When(p []byte) *WriterMockWriteExpectation {
	...
	expectation := &WriterMockWriteExpectation{
		...
		expectationOrigins: WriterMockWriteExpectationOrigins{origin: minimock.CallerInfo(1)},
	}
	...
	if mmWrite.WriteMock.defaultExpectation != nil {
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.p != nil && !minimock.Equal(*mm_want_ptrs.p, mm_got.p) {
				mmWrite.t.Errorf("WriterMock.Write got unexpected parameter p, expected at\n%s:\nwant: %#v\n got: %#v%s\n",
			...
			mmWrite.t.Errorf("WriterMock.Write got unexpected parameters, expected at\n%s:\nwant: %#v\n got: %#v%s\n",
...
func (m *WriterMock) MinimockWriteInspect() {
	for _, e := range m.WriteMock.expectations {
		if mm_atomic.LoadUint64(&e.Counter) < 1 {
			m.t.Errorf("Expected call to WriterMock.Write at\n%s with params: %#v", e.expectationOrigins.origin, *e.params)
		}
	}
	...
	if !m.WriteMock.invocationsDone() && afterWriteCounter > 0 {
		m.t.Errorf("Expected %d calls to WriterMock.Write at\n%s but found %d calls",
