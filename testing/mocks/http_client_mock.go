	...
	if mmDo.DoMock.defaultExpectation != nil {
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.req != nil && !minimock.Equal(*mm_want_ptrs.req, mm_got.req) {
				mmDo.t.Errorf("HTTPClientMock.Do got unexpected parameter req, want: %#v, got: %#v%s\n", *mm_want_ptrs.req, mm_got.req, minimock.Diff(*mm_want_ptrs.req, mm_got.req))
			}
...
func (m *HTTPClientMock) MinimockDoInspect() {
	...
	if !m.DoMock.invocationsDone() && afterDoCounter > 0 {
		m.t.Errorf("Expected %d calls to HTTPClientMock.Do but found %d calls",
			mm_atomic.LoadUint64(&m.DoMock.expectedInvocations), afterDoCounter)
	}
}
